...
const getAllPosts = async (req,res,next)=>{
    try {
        ...
        const posts= await Post.find({}).select("-__v").populate("author","-password -__v").populate("likedUsers").execPopulate()
...
const likedPost = async(req,res,next) =>{
    try {
        ...
        const posts=await Post.findById(postId).populate('likedUsers').execPopulate()
        res.status(201).json({
            posts
        })
